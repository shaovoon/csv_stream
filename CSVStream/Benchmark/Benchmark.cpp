...
#include "../CSVStream/csv_stream.h"
...
{
	const std::string mini_file = "F:\\mini_products.txt";
	const std::string capi_file = "F:\\capi_products.txt";
	const std::string capi_file2 = "F:\\capi_products2.txt";
	...
	//=========================
	{
		...
		csv::ofstream os(mini_file.c_str());
		os.set_delimiter(',', "$$");
		...
		csv::ifstream is(mini_file.c_str());
		is.set_delimiter(',', "$$");
	...
	}
	{
		...
		csv::ofstream os(capi_file.c_str());
		os.set_delimiter(',', "$$");
		os.enable_surround_quote_on_str(true, '\"');
		...
		csv::ifstream is(capi_file.c_str());
		is.set_delimiter(',', "$$");
		is.enable_trim_quote_on_str(true, '\"');
	...
	}
	{
		...
		while (ifs.read_line())
		{
			try
			{
				ifs >> product.name >> product.qty >> product.price;
			}
			catch (std::runtime_error& e)
			{
				fprintf(stderr, "%s\n", e.what());
